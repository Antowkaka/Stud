function AList(array){
    ...
    this.help = function () {
        ...
        let i = 1;
        for(let key in AList.prototype) console.log(i++, key);
    }
    this.init();
}
...
AList.prototype.addPos = function(index, value){
    ...
    for (let i = 0; i < this.size()+1; i++) {
        if(i >= index){
            i == index ? arrWithPosElem[i] = value : arrWithPosElem[i] = this.arr[i-1];
        }else arrWithPosElem[i] = this.arr[i];
    }
    this.arr = arrWithPosElem;
    return this.size();
}
